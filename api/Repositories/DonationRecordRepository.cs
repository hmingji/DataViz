...
namespace api.Repositories
{
    public class DonationRecordRepository : IDonationRecordRepository
    {
        ...
        public async Task<List<DonationRecord>> GetRecords(string state, DateOnly startDate, DateOnly endDate)
        {
            ...
            List<DonationRecord> records = (await connection.QueryAsync<DonationRecord>("SELECT * FROM DonationRecords WHERE State = @state AND Date BETWEEN @startDate AND @endDate", new { state = state, startDate = startDate, endDate = endDate })).ToList();
